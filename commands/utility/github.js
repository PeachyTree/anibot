...
module.exports = class GitHubCommand extends Command {
    ...
    async run(message, args) {
        ...
        if (searche[0].indexOf('/') !== -1) {
            ...
            const query = args ? args : 'anibot'
            const msg = await message.channel.send(`🔄 | Searching for **${query}**...`)

            const res = await snekfetch.get(`https://api.github.com/search/repositories?q=${query ? query.length < 2 ? query.join('+') : query : 'anibot'}`)
            if (res.status !== 200) {
